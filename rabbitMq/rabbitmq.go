...
func (mq *MessageQueue) SendMessage(message Message) {
	mq.declareQueue(mq.QueueName, nil)

	// 队列绑定到exchange
	mq.bindQueue(mq.QueueName, mq.RouteKey, mq.ExchangeName)
	...
	log.Printf("[body] Sent %s", message.Body)
	failOnError(err, "send srvutil msg err")
}
